...
RoleList::RoleList(std::string filename, std::vector<std::string> input,
  std::vector<ListEntry *> data, std::vector<Faction *> rq_factions)
{
  ...
  if (wincons.size() < 2)
    throw Error("Invalid rolelist");
  for (auto entry : roles) {
    if (entry->oppose != -1 && !wincons.contains(entry->oppose))
      throw Error("Invalid rolelist");
  }
...
void RoleList::generate()
{
  ...
  for (auto ientry : this->query) {
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      if (std::find(aliases.begin(), aliases.end(), ientry) != aliases.end()) {
        ...
        case ListEntry::Type::ROLE: {
          ...
          output.push_back(std::pair<ListEntry *, Role *>(
            data[i], static_cast<Role *>(data[i])));
    ...
    if (!is_valid)
      throw Error(filename, line, "Invalid role name: %s", ientry.c_str());
  }
  ...
  for (auto role : output) {
    if (role.second->wincon != -1) {
      if (!wincons.contains(role.second->wincon))
        wincons[role.second->wincon] = true;
    }

    auto it = std::find(
      rq_factions.begin(), rq_factions.end(), data[role.second->faction_pos]);
    if (it != rq_factions.end())
      rq_faction_counts[std::distance(rq_factions.begin(), it)]++;
  }
  ...
  for (int i = 0; i < rq_factions.size(); i++) {
    ...
    for (int j = 0; j < output.size(); j++) {
      if (rq_factions[i]->pos == output[j].second->faction_pos)
        faction_roles.push_back(j);
      if (std::find(rq_factions[i]->require.begin(),
            rq_factions[i]->require.end(), output[j].second)
        != rq_factions[i]->require.end()) {
        cont = true;
        break;
      }
    }
    ...
    if (rq_faction_counts[i] < rq_factions[i]->require_min)
      continue;
    ...
    case ListEntry::Type::ROLE:
      output[index].second
        = generate_role_from_role(rq_factions[i]->fallback->pos);
      break;
    case ListEntry::Type::ALIGNMENT:
      output[index].second
        = generate_role_from_alignment(rq_factions[i]->fallback->pos);
      break;
    }
  }
...
Role *RoleList::generate_role_from_role(int i)
{
  ...
  if (counts[role->faction_pos] == data[role->faction_pos]->limit)
    throw Error(filename, line, "Too many of faction: %s",
      data[role->faction_pos]->name.c_str());
  if (counts[role->alignment_pos] == data[role->alignment_pos]->limit)
    throw Error(filename, line, "Too many of alignment: %s",
      data[role->alignment_pos]->name.c_str());
  if (counts[i] == data[i]->limit)
    throw Error(filename, line, "Too many of role: %s", role->name.c_str());
...
Role *RoleList::generate_role_from_alignment(int i)
{
  ...
  if (counts[alignment->faction_pos] == data[alignment->faction_pos]->limit)
    throw Error(filename, line, "Too many of faction: %s",
      data[alignment->faction_pos]->name.c_str());
  if (counts[i] == data[i]->limit)
    throw Error(
      filename, line, "Too many of alignment: %s", alignment->name.c_str());
...
Role *RoleList::generate_role_from_faction(int i)
{
  ...
  if (counts[i] == data[i]->limit)
    throw Error(
      filename, line, "Too many of faction: %s", faction->name.c_str());
  counts[i]++;

  while (true) {
    Alignment *alignment
      = faction->alignments[std::rand() % faction->alignments.size()];
...
void RoleList::shuffle(std::vector<std::string> scrolls) { _shuffle(scrolls); }
...
void RoleList::_shuffle(std::vector<std::string> scrolls)
{
  ...
  for (auto pscrolls : scrolls) {
    ...
    for (auto str : strs) {
      for (auto e : data) {
        if (std::find(e->aliases.begin(), e->aliases.end(), str)
          != e->aliases.end()) {
        ...
        if (str[0] == '-') {
          ...
          if (std::find(e->aliases.begin(), e->aliases.end(), substr)
            != e->aliases.end()) {
  ...
  for (auto i : shuffled) {
    ...
    for (auto role : output) {
      ...
      for (auto e : bvec) {
        if (role.second->name == e->name)
          count += 90;
        if (data[role.second->alignment_pos]->name == e->name)
          count += 90;
        if (data[role.second->faction_pos]->name == e->name)
          count += 90;
      }
  ...
  for (int i = 0; i < output.size(); i++) {
    ...
    for (auto target : role->target_data) {
      while (true) {
        ...
        if (std::find(targets[i].begin(), targets[i].end(), index)
          != targets[i].end())
          continue;
        ...
        for (auto excl : target.exclude) {
          ...
          if (it != excl.end()) {
            int num
              = std::stoi(excl.substr(0, std::distance(excl.begin(), it)));
            std::string type = excl.substr(std::distance(excl.begin(), it) + 1);

            if (num >= targets[i].size())
            ...
            if (type == "alignment"
              && data[cf->alignment_pos] == target_alignment) {
              cont = true;
              break;
            }
